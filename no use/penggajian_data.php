...
// $query =mysqli_query($koneksi, "SELECT * FROM penggajian INNER JOIN total_harian on penggajian.id_teknisi=total_harian.id_teknisi where nama_teknisi like '%$cari%' limit $offset,$dataPerPage");
// $query = mysqli_query($koneksi, "SELECT * FROM penggajian where nama_teknisi like '%$cari%' limit $offset,$dataPerPage");

$query = mysqli_query($koneksi, "SELECT penggajian.*,user.nama,presensi_harian.tanggal,presensi_lembur.jam FROM $loc_penggajian where nama like '%$cari%' limit $offset,$dataPerPage");
    ...
    <div class="content-panel">
      ...
      <tbody>
        ...
        while ($result = mysqli_fetch_object($query)) {
          ...
          $total  = $result->gaji_harian*2 + $result->gaji_lembur*$result->jam + $result->gaji_bonus;
          ?>
          <tr>
